    ...
    if (options->screen_dither && is_power_of_two(options->screen_dither->size.width) && is_power_of_two(options->screen_dither->size.height)) {
        ...
        for (uint32_t y = 0; y < SCREEN_HEIGHT; ++y) {
            ...
            for (uint32_t x = 0; x < SCREEN_WIDTH / 8; ++x) {
                ...
                *display |= (colors[(bufferValue >= ditherBuffer[ditherYComp + (ditherX & maskX)])] << 7);
                bufferValue = buffer[++si];
                ditherX++;
                *display |= (colors[(bufferValue >= ditherBuffer[ditherYComp + (ditherX & maskX)])] << 6);
                bufferValue = buffer[++si];
                ditherX++;
                *display |= (colors[(bufferValue >= ditherBuffer[ditherYComp + (ditherX & maskX)])] << 5);
                bufferValue = buffer[++si];
                ditherX++;
                *display |= (colors[(bufferValue >= ditherBuffer[ditherYComp + (ditherX & maskX)])] << 4);
                bufferValue = buffer[++si];
                ditherX++;
                *display |= (colors[(bufferValue >= ditherBuffer[ditherYComp + (ditherX & maskX)])] << 3);
                bufferValue = buffer[++si];
                ditherX++;
                *display |= (colors[(bufferValue >= ditherBuffer[ditherYComp + (ditherX & maskX)])] << 2);
                bufferValue = buffer[++si];
                ditherX++;
                *display |= (colors[(bufferValue >= ditherBuffer[ditherYComp + (ditherX & maskX)])] << 1);
                bufferValue = buffer[++si];
                ditherX++;
                *display |= (colors[(bufferValue >= ditherBuffer[ditherYComp + (ditherX & maskX)])] << 0);
                ++display;
            }
            display += 2;
        }
